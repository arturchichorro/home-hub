...
- [ ] Split the recipe library and recipe detail into `/recipes` and `/recipes/$recipeId` routes
- [ ] Extend the recipe library query with ordered image metadata and preserve authorized signed-image reads
- [ ] Build the responsive recipe-card library with one mobile column, three desktop columns, and a card-sized create action
- [ ] Move title-only recipe creation into a focused dialog and navigate to the new detail page
- [ ] Add a validated, household-authorized optimistic mutation for recipe title and description updates
- [ ] Add borderless inline title and description editing with debounced saves, reversion, and error popovers
- [ ] Build the top gallery with all recipe images, immediate general-recipe uploads, compact progress, and an image viewer
- [ ] Build the responsive recipe-information layout with title and description beside the ingredient list
- [ ] Add the Shopping-style ingredient row with required name, optional quantity and unit, and no notes
- [ ] Redesign cooking history with a compact calendar-based add row, newest-first entries, comments, and linked image thumbnails
- [ ] Keep recipe, ingredient, cooking-log, and image deletion or additional editing actions out of this pass
- [ ] Verify loading, empty, optimistic, upload, failure, responsive, keyboard, and accessible-name behavior
- [ ] Update the design-system documentation and complete the Recipes UI checkpoint
